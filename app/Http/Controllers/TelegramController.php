...
{
    public function get_data_from_tg(Request $data){
        ...
        $message = mb_strtolower(($data['firstName'] ? $data['lastName']
            : $data['data']) , 'utf-8' );
        $method = 'sendMessage';
        switch ($message){
            case '/start':
    ...
    private function sendTelegram($method,$data,$headers=[]){
        ...
        $handle = curl_init(
'https://api.telegram.org/bot5509141318:AAHUhJUGaRw_NXx30gsUAJNCtaCysymSA0Q'
            .'/'.$method);
